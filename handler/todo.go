...
func IsTodoCompleted(w http.ResponseWriter, r *http.Request) {

	todo_id := chi.URLParam(r, "id")
	...
	iscompleted, err := dbHelper.IsCompletedTodoByID(todo_id, userCtx.ID)

	if err == sql.ErrNoRows {
		utils.ResponseError(w, http.StatusBadRequest, err, "todo not found!")
		return
	}

	if err != nil {
		utils.ResponseError(w, http.StatusInternalServerError, err, "failed to find todo!")
		return
	}

	err = dbHelper.MarkCompletedTodoByID(todo_id, userCtx.ID, iscompleted)
